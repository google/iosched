...
public class FirebaseDataReconciler {
    ...
    public FirebaseDataReconciler merge() {
        ...
        MergeHelper helper = new MergeHelper(mLocalUserData, mRemoteUserData,
                mMergedUserData);
        helper.mergeGCMKeys();
        helper.mergeUnsyncedActions(mActions);
    ...
    public FirebaseDataReconciler updateRemote() {
        if (remoteDataChanged()) {
            ...
            UpdateRemoteHelper helper = new UpdateRemoteHelper(mMergedUserData,
                    pendingFirebaseUpdatesMap);

            helper.updatePendingFirebaseUpdatesMap();
            FirebaseUtils.getUserDataRef(mContext, mAccountName).updateChildren(
                    pendingFirebaseUpdatesMap, new Firebase.CompletionListener() {
                        @Override
