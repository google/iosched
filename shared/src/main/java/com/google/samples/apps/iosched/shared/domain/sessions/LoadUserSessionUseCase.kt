...
) : FlowUseCase<Pair<String?, SessionId>, LoadUserSessionUseCaseResult>(defaultDispatcher) {

    override fun execute(
        parameters: Pair<String?, SessionId>
        ...
        return userEventRepository.getObservableUserEvent(userId, eventId).map {
            if (it is Success) {
                val useCaseResult = LoadUserSessionUseCaseResult(userSession = it.data.userSession)
