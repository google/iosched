 ...
 *
 * This class should only be used for data sources that emit a stream of results and are
 * [LiveData] aware.
 */
abstract class ObservableUseCase<in P, R> {
     ...
     *
     * Subclasses of this must ensure that result.postValue is called when new data is available.
     *
     * If subclasses, or classes they reference, hold a reference to the LiveData, they must
     * use weak references to avoid creating memory leaks.
     *
     * @param parameters the input parameters to run the use case with
     * @param result the MutableLiveData where the result is posted to
     *
     */
    operator fun invoke(parameters: P, result: MutableLiveData<Result<R>> = MutableLiveData()) {
    ...
    protected abstract fun execute(parameters: P, result: MutableLiveData<Result<R>>)
}

operator fun <R> ObservableUseCase<Unit, R>.invoke(): LiveData<Result<R>> = this(Unit)
operator fun <R> ObservableUseCase<Unit, R>.invoke(result: MutableLiveData<Result<R>>) = this(
        Unit, result
)
