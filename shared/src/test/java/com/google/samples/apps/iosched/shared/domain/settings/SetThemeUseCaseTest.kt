...
class SetThemeUseCaseTest {
    ...
    @Test
    fun `set theme`() = coroutineRule.runBlockingTest {
        val storage = FakePreferenceStorage()
        val useCase = SetThemeUseCase(storage, coroutineRule.testDispatcher)

        for (theme in Theme.values()) {
            useCase.invoke(theme)
            assertThat(storage.selectedTheme, equalTo(theme.storageKey))
        }
    }
}
